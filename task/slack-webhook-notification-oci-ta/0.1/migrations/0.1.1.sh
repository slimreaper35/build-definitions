...
TASK_SELECTOR="${TASKS_SELECTOR} | select(
  ...
  (.taskRef.params[] | (.name == \"name\" and .value == \"slack-webhook-notification\")) or
  (.taskRef.params[] | (.name == \"pathInRepo\" and (.value | test(\"task/slack-webhook-notification/\"))))
)"

if ! yq -e "${TASK_SELECTOR}" "$pipeline_file" >/dev/null 2>&1; then
  echo "slack-webhook-notification not found or already using git resolver, skipping"
  exit 0
fi
